...
const readUnicodeAt = (body, stringStart) => {
...
const collectUnicodeStringsInRange = (body, rangeStart, rangeEnd, chunkLengthOffsets) => {
    const hits = [];
    const view = new DataView(body.buffer, body.byteOffset, body.byteLength);
    let pos = rangeStart;
...
const walkChunk = (stream, body, ancestors) => {
    ...
    if (chunkType.startsWith("FOLD")) {
        ...
        hits.push(...collectUnicodeStringsInRange(body, dataStart, dataStart + chunkLength, nextAncestors));
    }
...
const spliceUnicodeString = (body, hit, newValue) => {
    ...
    rewritten.set(body.subarray(hit.stringEnd), hit.stringStart + encoded.length);
    if (delta !== 0) {
