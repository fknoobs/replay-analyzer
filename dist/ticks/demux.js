...
const parseActionsInBlock = (tick, data, startIndex, endIndex, tickDataStart, enrichCommands, out) => {
    let i = startIndex;
    let actionCount = 0;
    ...
    while (i + 2 <= endIndex &&
        ...
        if (i + actionLength > endIndex || i + actionLength > data.length)
            break;
        out.push(decodeAction(tick, data.subarray(i, i + actionLength), tickDataStart + i, actionLength, enrichCommands));
        actionCount++;
        i += actionLength;
    }
};
const parseTick = (data, tickDataStart, currentTickCount, enrichCommands, out) => {
    if (data.length < 16)
        return;
    const view = new DataView(data.buffer, data.byteOffset, data.byteLength);
    ...
    for (let i = 0; i < bundleCount; i++) {
        ...
        if (actionBlockSize > 0 && actionBlockSize < 65536) {
            ...
            if (actionEnd > data.length)
                break;
            parseActionsInBlock(tickId, data, offset, actionEnd, tickDataStart, enrichCommands, out);
            offset = actionEnd;
        }
        else {
            offset += 1;
...
const parseMessage = (stream, tick, chat) => {
    if (!stream.has(4))
        return false;
 ...
 */
export const parseTickStream = (stream, replay, enrichCommands) => {
    let tickIndex = 0;
    let tickCount = 0;
    while (stream.position < stream.length) {
        ...
        if (marker === 0) {
            ...
            if (tickLength === 0 || tickLength > 10000000)
                continue;
            if (!stream.has(tickLength))
                break;
            const tickDataStart = stream.position;
            const tickData = stream.readBytes(tickLength);
            parseTick(tickData, tickDataStart, tickCount, enrichCommands, replay.actions);
            tickCount++;
            ...
            if (!parseMessage(stream, tickIndex, replay.chat))
                break;
        }
        else {
            continue;
