...
const parseDataChunk = (stream, header, players, state, type, version) => {
...
const parseChunk = (stream, header, players, state) => {
    ...
    if (chunkType.startsWith("FOLD")) {
        while (stream.position < startPosition + chunkLength) {
            if (!parseChunk(stream, header, players, state))
                break;
        }
    }
...
const parseChunky = (stream, header, players, state) => {
 ...
 */
export const parseHeaderInto = (stream, replay) => {
