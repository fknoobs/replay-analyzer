...
function buildCohBody(upload: Uint8Array): {
    ...
    const { bytes, rewritten } = prepareForLocalCoh(upload, {
    ...
    try {
        ...
        setTimeout(() => {
            ...
            try {
                ...
                baselineReplayName = parseHeader(cohBodyBytes!).header.replayName;
                ...
                if (pathRewrites.length > 0) {
                    ...
                    notes.push(
                        "foreign workshop path found — set Local Documents then Reset",
                    );
                }
...
applySteamIdsBtn.addEventListener("click", () => {
    ...
    for (const input of steamInputs) {
        ...
        if (name && steamId) steamIdsByName[name] = steamId;
    }
    ...
    for (const select of idSelects) {
        ...
        if (name && raw) {
            const id = Number(raw);
            if (Number.isInteger(id) && id !== 0) playerIdsByName[name] = id;
        }
    }
    ...
    try {
        ...
        statusDiv.textContent = `Wrote "${currentReplay.header.replayName}", Steam ${linkedSteam}/${currentReplay.players.length}, IDs ${linkedIds}/${currentReplay.players.length}.`;
...
resetMetadataBtn.addEventListener("click", () => {
    ...
    try {
        ...
        currentReplay = parseCurrentFile();
        baselineReplayName = currentReplay.header.replayName;

        const pathNote =
            pathRewrites.length > 0
                ? `; rewrote ${pathRewrites.length} map path(s)`
                : "";
        statusDiv.textContent = `Exported CoH-compatible body (no FKSTMETA${pathNote})`;
...
function parseCurrentFile(): Replay {
    if (!currentFileBytes) throw new Error("No replay file loaded");
    if (currentMode === "header") return parseHeader(currentFileBytes);
...
function renderSteamIdInputs(replay: Replay) {
    steamIdRows.innerHTML = "";
    replayNameInput.value = replay.header.replayName;
    ...
    if (unresolved.unassignedPlayers.length > 0) {
        ...
        ambiguityHint.textContent = unresolved.unclaimedIds.length
            ? `Ambiguous player IDs: pick an ID for unassigned teammates (${idHints}).`
    ...
    for (const player of replay.players) {
        ...
        cpmEl.textContent =
            currentMode === "header"
                ? "CPM —"
                : `CPM ${playerCpm(replay, player.id)}`;
...
function displayResult(data: Replay) {
    ...
    if (dataForJson.actions.length > 100) {
        displayData = {
            ...dataForJson,
            actions: [
                ...dataForJson.actions.slice(0, 50),
                `... ${dataForJson.actions.length - 50} more actions hidden ...`,
            ],
        };
    }

    const pre = document.createElement("pre");
    pre.innerHTML = syntaxHighlight(JSON.stringify(displayData, null, 2));
