...
const main = () => {
    ...
    let actionDefinitions: ActionDefinitions | undefined;
    if (fs.existsSync(cohraPath)) {
        console.log(`Loading action definitions from ${cohraPath}...`);
        actionDefinitions = new ActionDefinitions(cohraPath);
    } else {
        console.warn("cohra.dat not found. Action descriptions will be missing.");
    }

    console.log(`Parsing ${filePath}...`);
    const parser = new ReplayParser(filePath, actionDefinitions);
