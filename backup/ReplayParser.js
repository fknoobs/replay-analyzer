...
const ReplayStream_1 = require("./ReplayStream");
const Replay_1 = require("./Replay");
class ReplayParser {
    constructor(filePath, actionDefinitions) {
        this.stream = new ReplayStream_1.ReplayStream(filePath);
        this.replay = new Replay_1.Replay(actionDefinitions);
    }
    parse() {
        try {
            this.parseHeader();
    ...
    parseChunk() {
        if (this.stream.position + 8 > this.stream.length)
            return false;
    ...
    processDataChunk(type, version) {
        // console.log(`Chunk: ${type} v${version}`);
    ...
    parseData() {
        let tickIndex = 0;
        // console.log(`Starting parseData at position ${this.stream.position}`);
        while (this.stream.position < this.stream.length) {
            if (this.stream.position + 4 > this.stream.length)
                break;
    ...
    parseTick(data, tickDataStart) {
        if (data.length < 16)
            return;
        const tickId = data.readUInt32LE(0);
        // console.error(`Tick ${tickId}`);
    ...
    parseActionsInBlock(tick, data, startIndex, endIndex, tickDataStart) {
        let i = startIndex;
        ...
        while (i + 2 <= endIndex && actionCount < maxActions) {
            ...
            if (actionLength <= 0 || actionLength > 1000)
                break;
            // if (i + actionLength > endIndex) break; // Relaxed check for parsing
