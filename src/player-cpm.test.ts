...
describe("playerCpm", () => {
    it("returns 0 for missing player / empty duration", () => {
        ...
        expect(playerCpm({ ...replay, duration: 0 }, replay.players[0]?.id)).toBe(
            0,
        );
    ...
    it("is far below raw action rate on the main fixture", () => {
        ...
        const mins = replay.duration / 60;
        for (const p of replay.players) {
            if (p.id == null) continue;
            const raw = replay.actions.filter((a) => a.playerID === p.id).length;
    ...
    it("stops counting at AI takeover and ignores takeover itself", () => {
        ...
        // Early dropout → C2A reports CPM 0 (no human commands before takeover).
    ...
    it("matches Replay Manager / C2A reference values when sample replays exist", () => {
        ...
        for (const { file, expected } of cases) {
            ...
            for (const [name, cpm] of Object.entries(expected)) {
                ...
                // Allow ±1 for integer rounding edge cases vs C2A.
                expect(Math.abs(playerCpm(replay, player!.id) - cpm)).toBeLessThanOrEqual(
                    1,
                );
            }
        }
        if (ran === 0) {
            // Fixtures-only CI: skip without failing.
            expect(ran).toBe(0);
        }
    });
});
