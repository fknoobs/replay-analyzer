 ...
 */
export type DataChunkHandler<T> = (
    chunkType: string,
    chunkVersion: number,
    dataStart: number,
    chunkLength: number,
 ...
 */
export const encodeLengthPrefixedUnicode = (value: string): Uint8Array => {
 ...
 */
export const tryEnterChunky = (stream: BinaryReader): boolean => {
 ...
 */
export const walkChunky = <T>(
    stream: BinaryReader,
    ancestors: number[],
    onDataChunk: DataChunkHandler<T>,
): ChunkyWalkResult<T> => {
 ...
 */
export const forEachHeaderDataChunk = (
    body: Uint8Array,
    onDataChunk: DataChunkHandler<unknown>,
): void => {
 ...
 */
export const findInHeaderChunky = <T>(
    body: Uint8Array,
    onDataChunk: DataChunkHandler<T>,
): T | null => {
    const stream = new BinaryReader(body);
 ...
 */
export const spliceLengthPrefixedUnicode = (
    body: Uint8Array,
    location: UnicodeStringLocation,
    value: string,
): Uint8Array => {
 ...
 */
export const readLengthPrefixedUnicodeAt = (
    body: Uint8Array,
    stringStart: number,
): string => {
