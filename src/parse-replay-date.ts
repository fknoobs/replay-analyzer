...
const buildMeridiemAlternation = (markers: string[]) =>
    [...markers]
...
const normalizeMeridiem = (raw: string | undefined): "AM" | "PM" | undefined => {
    if (!raw) return undefined;
    const trimmed = raw.replace(/\u202F/g, " ").replace(/\s+/g, " ").trim();
    const compact = trimmed.replace(/\s/g, "").toUpperCase();

    // Fast Latin paths
    if (/^A\.?M\.?$/.test(compact) || compact === "A") return "AM";
    if (/^P\.?M\.?$/.test(compact) || compact === "P") return "PM";

    for (const m of MERIDIEM_AM) {
        if (m.toUpperCase() === trimmed.toUpperCase() || m === trimmed) return "AM";
        if (m.replace(/\s/g, "").toUpperCase() === compact) return "AM";
    }
    for (const m of MERIDIEM_PM) {
        if (m.toUpperCase() === trimmed.toUpperCase() || m === trimmed) return "PM";
        if (m.replace(/\s/g, "").toUpperCase() === compact) return "PM";
    }
    return undefined;
};

const applyMeridiem = (hour: number, meridiem: string | undefined): number | null => {
...
const resolveDayMonth = (
    ...
    if (a > 12) return b <= 12 ? { day: a, month: b } : null;
    if (b > 12) return { day: b, month: a };

    if (hasMeridiem && separator === "/") return { day: b, month: a }; // MDY
    return { day: a, month: b }; // DMY
};

type TimeParts = { hour: number; minute: number; second: number; meridiem?: string };

const parseTimeAndMeridiem = (timeRaw: string): TimeParts | null => {
    const trimmed = timeRaw.trim();

    // Meridiem before time (ko/zh/ja): "오후 6:12", "下午 06:12"
    const before = trimmed.match(
        new RegExp(
            `^(${MERIDIEM_ANY_RE})\\s*(\\d{1,2})[:.](\\d{2})(?:[:.](\\d{2}))?$`,
            "i",
        ),
    );
    if (before && normalizeMeridiem(before[1])) {
        ...
        if (adjusted === null || minute > 59 || second > 59) return null;
    ...
    const after = trimmed.match(
        new RegExp(
            `^(\\d{1,2})[:.](\\d{2})(?:[:.](\\d{2}))?\\s*(${MERIDIEM_ANY_RE})?$`,
            "i",
        ),
    );
    ...
    if (meridiem && !normalizeMeridiem(meridiem)) return null;
 ...
 */
const normalizeNumericDatePart = (datePart: string): string => {
    let s = datePart.trim();
...
const parseNumericDate = (raw: string): string | null => {
    const hadWeekday = /\([^)]*\)/.test(raw);
    const cleaned = stripDecorations(raw);

    const timeSep = cleaned.search(
        new RegExp(`\\s+(?:(?:${MERIDIEM_ANY_RE})\\s*)?\\d{1,2}[:.]\\d{2}`, "i"),
    );
    if (timeSep === -1) return null;
    ...
    const meridiemIsCjk = Boolean(
        time.meridiem && /^(오전|오후|午前|午後|上午|下午)$/.test(time.meridiem),
    );
    ...
    if (yearFirst) {
        ...
        if (yearToken.length > 2 || acceptTwoDigitYear) {
            const formatted = formatIsoLocal(
                year,
                month,
                day,
                time.hour,
...
const parseCjkKanjiDate = (raw: string): string | null => {
    const match = raw.match(
        new RegExp(
            `^(\\d{4})\\s*年\\s*(\\d{1,2})\\s*月\\s*(\\d{1,2})\\s*日\\s*(?:(${MERIDIEM_ANY_RE})\\s*)?(\\d{1,2})[:.](\\d{2})(?:[:.](\\d{2}))?$`,
            "i",
        ),
    );
...
const parseMonthNameDate = (raw: string): string | null => {
    const monthFirst = raw.match(
        new RegExp(
            `^([A-Za-z]+)\\s+(\\d{1,2}),?\\s+(\\d{4})\\s+(\\d{1,2}[:.]\\d{2}(?:[:.]\\d{2})?(?:\\s*(?:${MERIDIEM_ANY_RE}))?)$`,
            "i",
        ),
    );
    if (monthFirst) {
    ...
    const dayFirst = raw.match(
        new RegExp(
            `^(\\d{1,2})\\s+([A-Za-z]+)\\s+(\\d{4})\\s+(\\d{1,2}[:.]\\d{2}(?:[:.]\\d{2})?(?:\\s*(?:${MERIDIEM_ANY_RE}))?)$`,
            "i",
        ),
    );
    if (dayFirst) {
...
export const parseReplayDate = (raw: string): string => {
    const normalized = normalizeDateString(raw);
    if (!normalized) return raw;

    return (
        parseCjkKanjiDate(normalized) ??
