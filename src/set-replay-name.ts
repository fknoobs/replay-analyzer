...
type ReplayNameLocation = {
    /** Offset of the u32 length prefix of replayName. */
    stringStart: number;
    /** Offset just past the UTF-16LE payload. */
    stringEnd: number;
    /** Offsets of ancestor FOLD/DATA chunkLength u32 fields (inclusive of DATABASE). */
    chunkLengthOffsets: number[];
};

type WalkResult = {
    location: ReplayNameLocation | null;
    /** False when the next bytes are not a FOLD/DATA chunk (mirrors parseChunk). */
    advanced: boolean;
};
    ...
    if (chunkType.startsWith("FOLD")) {
        ...
        if (dataStart + DATABASE_PREFIX_BYTES + 4 > stream.length) {
            throw new RangeError(
                "DATABASE chunk too short to contain replayName",
            );
        }
        ...
        if (charCount * 2 > stream.remaining()) {
            throw new RangeError(
                `replayName length ${charCount} exceeds remaining bytes`,
            );
        }
...
const locateReplayName = (body: Uint8Array): ReplayNameLocation => {
    ...
    throw new Error(
        "Could not locate DATABASE replayName in replay header",
    );
};
