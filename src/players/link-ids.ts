 ...
 */
export const isPlayerUnassigned = (player: { id?: number }): boolean =>
 ...
 */
export const buildIdFactionMap = (
 ...
 */
export const inferIdFaction = (
    actions: readonly Action[],
    id: number,
): string | undefined => {
    for (const action of actions) {
        if (action.playerId !== id || !isUnit(action.commandId)) continue;
        if (US_UNITS.has(action.objectId)) return "allies";
        if (CW_UNITS.has(action.objectId)) return "allies_commonwealth";
        if (WEHR_UNITS.has(action.objectId)) return "axis";
        if (PE_UNITS.has(action.objectId)) return "axis_panzer_elite";
    }
    return undefined;
};
 ...
 */
export const applyRelicLadderPlayers = (
    players: Player[],
    ladder: RelicLadderPlayer[] | undefined,
): void => {
    if (!ladder || ladder.length === 0) return;
    if (ladder.length !== players.length) return;
 ...
 * @param actions - Action stream to scan (single pass).
 */
export const attachPlayerDoctrines = (
    players: Player[],
    actions: readonly Action[],
): void => {
    const doctrineByPlayer = new Map<number, number>();
    for (const action of actions) {
        if (!isDoctrinal(action.commandId)) continue;
        if (doctrineByPlayer.has(action.playerId)) continue;
 ...
 */
export const linkPlayerIds = (replay: Replay): void => {
    ...
    const assignUnique = (players: Player[], ids: number[]): void => {
...

    {
        ...
        const remainingIds = actionPlayerIds.filter(
