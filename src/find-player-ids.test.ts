...
import { applyPlayerIds } from "../src/apply-player-ids";
import { getUnresolvedPlayerIds } from "../src/player-id-ambiguity";
import { embedPlayerIds } from "../src/replay-metadata";
...
describe("player ID linking", () => {
    it("does not guess IDs for same-faction teammates without chat", function () {
        if (!fs.existsSync(localProblemReplay)) {
            this.skip();
        }
        ...
        expect(unresolved.unassignedPlayers.map((p) => p.name).sort()).toEqual(
            ["CamoFILMs", "EGY | GAZA"].sort(),
        );
    ...
    it("applyPlayerIds can correct ambiguous teammates and doctrines", function () {
        if (!fs.existsSync(localProblemReplay)) {
            this.skip();
        }
    ...
    it("embedPlayerIds persists corrections across parseReplay", function () {
        if (!fs.existsSync(localProblemReplay)) {
            this.skip();
        }
        ...
        expect(
            replay.players.find((p) => p.name === "EGY | GAZA")?.doctrineName,
        ).toBe("Armor");
        expect(
    ...
    it("keeps chat-confirmed IDs for same-faction teammates", function () {
        if (!fs.existsSync(chatConfirmedReplay)) {
            this.skip();
        }
    ...
    it("uses 1000+slot on fixed-start games (COHRA model)", function () {
        const fixed = path.resolve(
            __dirname,
            "../VIRE RIVER VALLEY (8)  .2026-02-05.00-40-53.rec",
        );
        if (!fs.existsSync(fixed)) this.skip();

        const replay = parseReplay(readFixture(fixed));
        expect(replay.randomStart).toBe(false);
...
describe("applyPlayerIds", () => {
    it("matches case-insensitively and refreshes doctrines", () => {
        ...
        expect(replay.actions[0].playerName).toBe("Alice");
        expect(replay.actions[1].playerName).toBe("Bob");
    });
});
