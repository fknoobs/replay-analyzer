...
/** @internal */
const isMapArchivePath = (value: string): boolean =>
    ABS_WIN_PATH_RE.test(value) &&
 ...
 * @internal
 */
const locateMapArchivePaths = (body: Uint8Array): UnicodeStringLocation[] => {
    const hits: UnicodeStringLocation[] = [];

    findInHeaderChunky(
        body,
        (chunkType, chunkVersion, dataStart, chunkLength, chunkLengthOffsets) => {
...
): { path: string; stringStart: number; stringEnd: number }[] => {
 ...
 * // Hand `bytes` to CoH
 */
export const prepareForLocalCoh = (
    input: ArrayBuffer | Uint8Array,
    options: PrepareForLocalCohOptions = {},
): PrepareForLocalCohResult => {
