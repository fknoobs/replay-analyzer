...
} from "../metadata/fkstmeta";
import {
    findInHeaderChunky,
    spliceLengthPrefixedUnicode,
    type UnicodeStringLocation,
} from "../chunky/walk";
 ...
 * @internal
 */
const locateReplayName = (body: Uint8Array): UnicodeStringLocation => {
    const location = findInHeaderChunky(
        body,
        (chunkType, chunkVersion, dataStart, _len, chunkLengthOffsets, stream) => {
...
export const setReplayName = (
    ...
    const rewritten = spliceLengthPrefixedUnicode(
        body,
        locateReplayName(body),
        replayName,
    );
