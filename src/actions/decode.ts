 ...
 */
export const resolveStaticCommand = (
    commandId: number,
    objectId: number,
): Command | undefined => {
    switch (commandId) {
...
export const extractPosition = (
    ...
    const tryAt = (i: number): Vec3 | undefined => {
...
export const decodeAction = (
    ...
    const position =
        resolved !== undefined && COMMANDS_WITHOUT_POSITION.has(resolved.type)
            ? undefined
