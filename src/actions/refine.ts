...
type UnitCommandDef = {
    name: string;
    description: string;
    factionVariants?: Record<string, { name: string; description: string }>;
};
 ...
 */
export const refineActionDefinitions = (
    actions: Action[],
    players: Player[],
): void => {
    ...
    for (const action of actions) {
        ...
        if (isUnitCommand(action.commandId)) {
            ...
            const baseDef = (DEFINITIONS.UNIT_COMMAND as Record<
                number,
                UnitCommandDef | undefined
            >)[action.objectId];
